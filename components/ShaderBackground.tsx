  ...
  useEffect(() => {
    ...
    const fsSource = `
      ...
      void main() {
          ...
          vec3 color1 = vec3(0.07, 0.06, 0.05);
          vec3 color2 = vec3(0.40, 0.28, 0.16);
          vec3 color3 = vec3(0.12, 0.10, 0.08);
          
          float factor = sin(u_time * 0.2 + length(uv - 0.5) * 5.0 + noise) * 0.5 + 0.5;
          vec3 color = mix(color1, color3, factor);
          
          float dist = length(uv - mouse);
          float glow = smoothstep(0.4, 0.0, dist);
          color = mix(color, color2, glow * 0.3);
